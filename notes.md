...
This file records a targeted bug-fix pass and replaces the previous project README (moved here).
...
- `fastf1` (optional, for F1 telemetry integration)
- `requests` (optional, for OpenF1 API)
- `scipy` (optional, for physics calibration)
...
- F1 calibration (`simulator/calibrate_from_f1.py`)
...
5. `utils/`
- helpers for YAML loading, JSON writing, FastF1 data loading (`f1_data_loader.py`), OpenF1 API bridge (`openf1_bridge.py`)
...
```bash
# Fetch F1 data via FastF1
python utils/f1_data_loader.py --year 2023 --gp Monza --session R --save

# Stream live F1 telemetry via OpenF1
python utils/openf1_bridge.py live --session-key <key>

# Calibrate physics model against real telemetry
python simulator/calibrate_from_f1.py --lap-file data/logs/f1_VER_*.json --calibrate --report
```

### D. Live workflow example
...
# FSAE Telemetry Simulator — Notes

## Current Status (2026-07-16)

### What's Built
- **Simulator**: Real-time (10 Hz) + multi-lap race generator with physics (simple), sensor models, driver profiles, track loader
- **Streamlit Dashboard**: 19 pages — real-time telemetry, data/lap visualization, track designer, session replay, driver analysis/comparison, live recommendation, driver assistant
- **Recommendation Engine**: Live recommendation with policy-based coaching, driver aggregate metrics, driver feature extraction
- **Driver Analysis**: Driver profiles, comparison tables, metric explanations, live assistant
- **F1 Integration** (new):
  - `utils/f1_data_loader.py` — FastF1 session fetcher, telemetry extraction, conversion to project log format, lap comparison/validation
  - `utils/openf1_bridge.py` — OpenF1 API client for sessions, car data, laps, weather, positions; live streaming to realtime.json
  - `simulator/calibrate_from_f1.py` — Physics calibration pipeline: tune car parameters (mass, drag, engine force, etc.) against real F1 telemetry via scipy optimization

### Git
- **Remote**: `origin` → `https://github.com/darisdzakwanhoesien2/fsae-simulator-dashboard.git`
- **Branch**: `main` (tracks `origin/main`)
- **Last commit**: `79e1cd8` — "F1 integration: FastF1 loader, OpenF1 bridge, physics calibration pipeline"

## Things To Do

### Short-term
- [ ] Clean up duplicate page numbers (e.g., two `12_Driver_*` files, two `1_Realtime_Telemetry.py` locations)
- [ ] Remove `__pycache__` directories from version tracking (add to `.gitignore`)
- [ ] Remove old/unused data files in `old_code/` and `data/logs/`
- [ ] Resolve discrepancies between `project_directory.md` and actual directory structure
- [ ] Standardize file naming across `simulator/` (mix of snake_case conventions)

### Longer-term
- [x] FastF1 integration — session fetcher, telemetry loader, lap comparison
- [x] OpenF1 API integration — car data, laps, weather, live streaming
- [x] Physics calibration pipeline — tune car params from real telemetry
- [ ] Behavioral Cloning (imitation learning) from driver logs
- [ ] Driver style clustering (k-means / HDBSCAN)
- [ ] Offline RL (CQL/IQL) or online RL (PPO/SAC) training
- [ ] Adaptive live recommendation per driver cluster
- [ ] GPS-based track maps (CSV/GPX import)
- [ ] G-G acceleration plot
- [ ] Lap time prediction via ML
- [ ] CAN-Bus ingestion module
- [ ] MQTT / WebSocket live telemetry
- [ ] Sector times (S1/S2/S3) and corner detection
